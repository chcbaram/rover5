...
int Tracking_Red( THREAD_OBJ *pArg )
{
	...
	while ( pArg->Thread_Stop == FALSE )
	{
		...
		unsigned short *pImgLcd = (unsigned short *)imgLcd->imageData;
		...
		for( y=0; y<imgLcd->height; y++ )
		{
			y_offset = (y+y_pos) * vinfo.xres;
			
			for( x=0; x<imgLcd->width; x++ )
			{
				pfb[y_offset + x + x_pos] = pImgLcd[ y*imgLcd->width + x];
			}
		}
		...
		pImgLcd = (unsigned short *)imgLcd->imageData;
		...
		cvCvtColor( imgLabel, imgLcd_Resize, CV_GRAY2BGR565 ); 
		cvResize(imgLcd_Resize, imgLcd);
...
/*
		...
		pImgLcd = (unsigned short *)imgLcd_Resize->imageData;
		pfb     = (unsigned short *)fbp;


		x_pos = imgLcd->width + 10;
		y_pos = 0;

		for( y=0; y<imgLcd_Resize->height; y++ )
		{
			y_offset = (y+y_pos) * vinfo.xres;
			
			for( x=0; x<imgLcd_Resize->width; x++ )
			{
				pfb[y_offset + x + x_pos] = pImgLcd[ y*imgLcd_Resize->width + x];
			}
		}
*/
